...
if opts.config then
	fyaml:set_path(args[opts.config])
end

local DAEMON = "tmpod"
local DEVICE = fyaml:get("system", "device")
...
local TMPO_DECIMATION_FACTOR = fyaml:get("tmpo", "decimation") or 8
math.randomseed(nixio.bin.crc32(DEVICE .. os.time()))
local TMPO_CLOSE8_GRACE = math.floor(4 + math.random() * 64) -- 4-68 secs
local TMPO_BASE_PATH = fyaml:get("tmpo", "root") .. "/tmpo/sensor/"
...
local config = {
	...
	load = function(self)
		...
		mqtt:publish(MOSQ_TOPIC_SENSOR_CONFIG, jsensor, MOSQ_QOS1, MOSQ_RETAIN)
...
mqtt.ON_MESSAGE = function(mid, topic, jpayload, qos, retain)
	...
	elseif topic == TMPO_TOPIC_SYNC_SUB then
		...
		sensor(topic:match(TMPO_REGEX_SENSOR))
	end
end
